...
class FAP(object):
    def __init__(self):
        ...
        self.y = 0.0
        self.cap = 0
        self.cf = 10 ** (-30)  # the computation constant factor for F-AP
        self.comp_limit = 0
...
class FranEnv(object):
    def __init__(self, UE_nm, FAP_nm):
        ...
        self.link_cf = 10 ** (-8)                     #the computation constant factor for fronthaul links
        self.cloud_cf =  1.5 * (10 ** (-30))            #the computation constant factor for cloud
    ...
    def dqn_step(self, cur_state, action_number, state_len):
        W = 20 * (10 ** 6)  # channel bandwidth, in Hz
        ...
        for ue_index in range(self.UE_nm):
            ...
            for b in range(0, 4):
                if self.ue[ue_index].u2f_cq[1][b] == cur_state[ue_index][self.UE_nm * self.FAP_nm + 1]:
                    break
            ...
            for q in range(0, 4):
                if self.ue[ue_index].u2f_cq[2][q] == cur_state[ue_index][self.UE_nm * self.FAP_nm + 2]:
                    break
...
class DQN(object):
    ...
    def create_model(self, nb_actions, observation_shape):
        ...
        model.add(Dense(32, activation="relu"))  # the second layer
